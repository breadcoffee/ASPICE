...
## 공급업체 모니터링 프로세스 ACQ4
- 공급 업체 모니터링이란?
    - 공급 업체 계약에 명시된 사항의 이행에 대한 검토 및 관리를 통해 성공적인 프로젝트 진행이 되도록 함
- 공급 업체 & 협력 업체란?
    - 발주자에서 제품 및 서비스를 납품하는 존재임
    - 발주자와 계약 후 설계, 개발, 제조, 유지보수 또는 솔루션의 공급 등을 합의한 개인, 법인, 단체 등을 의미함
    - 공급 업체가 제출하는 인도물(결과물)은 하드웨어, 소프트웨어, 문서, 서비스, 솔루션 등의 산출물을 의미함
    - ACQ. 4 적용 대상이 아닌 경우
        - 공급업체에서 인력 제공만 하는 경우로 고용된 인원이 발주자의 프로세스를 따라 개발하는 경우
        - COTS(상용제품) 제품을 납품하고 유지보수 등의 변경이 없는 경우
...
- 공급업체 모니터링 성과
    ...
    - 합의사항에 대한 변경협의 및 문서화
- Base Practices
    - BP 1 : Agree on and maintain joint processes, joint interfaces, and information to be exchanged. [상호간의 공동 프로세스, 공동 인터페이스, 교환되어야할 정보를 합의하고 유지한다.]
        - 설명
            - 공급업체 계약 내용을 토대로 **공급업체 관리 계획**을 수립하고 **모든 이해관계자가 요구사항을 이해하고 동의**하는지 확인한다.
            - 공급업체와의 원활한 협업을 위해서 **프로젝트 관리, 요구사항 관리, 변경 관리, 형상 관리, 문제 해결 관리, 품질 보증, 고객 승인 등의 프로세스**를 통해서 **인터페이스** 한다
        - Output Work Produce
            - Commitment/agreement(공급 업체와의 합의)
                - Customer(발주자)
                    - 공급업체의 프로젝트 수행범위
                    - 공급업체와의 공동으로 수행할 프로세스
                    - 공급업체와의 공동수행절차 및 인터페이스
                    - 공급업체와 교환해야하는 정보
                - Supplier
                    - 역할 및 책임과 이해관계자
                    - 문제해결 및 변경 관리방안
                    ...
                    - 테스트표준과 기준
                    - 에스컬레이션 메커니즘
            - Communication record(공급업체 관리 계획서)
                - 공급업체 관리 개요
                - 공급업체 관리 조직
                - 공급업체 개발 전략
                - 공급업체 관리 계획
                - ...
        - Practice Checklist
            - 공급업체와 어떤 프로세스를 통해서 인터페이스 하고 있는가?
            - 공급업체와 합의하는 사항은 무엇이 있는가?
    - BP 2 : Exchange all agreed information. [합의된 모든 정보를 교환한다.]
        - 설명
            - **공급업체와 보안 협약한 산출물에 대해서는 공유**함을 원칙으로 한다.
            - 공급업체와 **공유한 산출물 및 회의 결과를 기록**한다
        - Output Work Product
            - Acceptance record
            - Communication record
        - Practice Checklist
            - 공급업체와 공유하는 산출물은 무엇인가?
            - 공급업체와 회의한 결과는 어떻게 관리하고 있는가?
    - BP 3 : Review technical development with the supplier. [공급업체와 함께 기술 개발을 점검한다.]
        - 설명
            - 공급업체의 산출물을 검토하기 위한 **점검 기준(표준 준수, 요구사항 준수 등)을 정의**한다.
            - 공급업체와 관련된 **기술적 이슈와 리스크에 대해서 정기적으로 검토**하고, 책임을 명확히 한다.
            - 공급업체와 관련된 기술적 이슈와 리스크에 대해서 종료될 때까지 **추적 관리**한다.
        - Output Work Product
            - Review record
            - Communication record
        - Practice Checklist
            - 공급업체와 관련된 기술적 이슈 및 리스크는 어떻게 식별하고 있는가?
            - 식별한 기술적 이슈와 리스크는 어떻게 추적 관리하고 있는가?
    - BP 4 : Review progress of the supplier. [공급업체의 진척 상황을 점검한다.]
        - 설명
            - 공급업체의 **품질, 비용, 일정 등의 진척 현황**을 관리하기 위한 기준을 정의한다.
            - 공급업체와 관련된 관리적 **이슈와 리스크에 대해서 정기적으로 검토**하고, 책임을 명확히 한다.
            - 공급업체와 관련된 관리적 이슈와 리스크에 대해서 종료될 때까지 **추적 관리**한다.
        - Output Work Product
            - Review record
            - Communication record
        - Practice Checklist
            - 공급업체의 진척 현황에 대해 무엇을 관리하고 있는가?
            - 공급업체와 관련된 관리적 이슈와 리스크는 어떻게 식별하고 있는가?
            - 식별한 관리적 이슈와 리스크는 어떻게 추적 관리하고 있는가?
    - BP 5 : Act to correct deviations. [편차(이슈)를 해결하기 위한 시정조치 활동을 수행한다.]
        - 설명
            - 협의한 사안에 대해서 이슈가 불가피한 경우, **적절한 시정조치(계약 변경, 변경 관리 등)를 수행**한다.
            - 필요한 경우 공급업체 계약서, 프로젝트 계획서 등을 변경한다.
            - **변경사항에 대해서는 재 합의**하고, **이슈가 재발하지 않도록 충분한 검토**를 수행한다.
        - Output Work Product
            - Change request
            - Corrective action register
        - Practice Checklist
            - 협의한 사안에 이슈가 생긴 경우, 어떤 조치를 취하는가?
            - 이슈가 재발하지 않도록 어떤 조치를 취하는가?
            - 에스컬레이션 체계는 어떻게 정의되어 있는가?